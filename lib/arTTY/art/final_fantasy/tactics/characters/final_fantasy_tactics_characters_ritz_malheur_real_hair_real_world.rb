class ArTTY::Art::FinalFantasyTacticsCharactersRitzMalheurRealHairRealWorld < ArTTY::Art
    def initialize
        super
        @colors = [
            "      8888      ",
            "    88a77a88    ",
            "   8177bb7718   ",
            "  81711abb778   ",
            "  8b14647baba8  ",
            "  87166647ab78  ",
            "  8a99664a1bb8  ",
            "  81a869911b718 ",
            "   87868a91baa8 ",
            "   846b8b9a7178 ",
            "  8a9466497a978 ",
            " 8ab192229719a8 ",
            " 8ba1abba9a1a18 ",
            " 841b111a11bb18 ",
            "  8ab030ba1bba8 ",
            "  8ba050bb1bbb8 ",
            " 8bb0353bb1abbb8",
            " 8b335553ba144b8",
            "  8035353bb1468 ",
            "   8033553bb198 ",
            "   82409903bba8 ",
            " 88892988442888 ",
            "8882428882428888",
            "8888888889298888",
            "8888888882428888",
            " 88888888448888 ",
            "    88888888    ",
        ]
        @name = "final-fantasy-tactics-characters-ritz-malheur-real-hair-real-world"
        map_color("0", "color_092")
        map_color("1", "color_095")
        map_color("2", "color_130")
        map_color("3", "color_141")
        map_color("4", "color_172")
        map_color("5", "color_183")
        map_color("6", "color_220")
        map_color("7", "color_225")
        map_color("8", "color_233")
        map_color("9", "color_238")
        map_color("a", "color_250")
        map_color("b", "color_255")
    end
end
